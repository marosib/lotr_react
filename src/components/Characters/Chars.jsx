...
export default function Chars({characters}) {
  ...
  const [maxPageNumber, setMaxPageNumber] = useState(Math.ceil(characters.length/pageSize))
 ...
 const searchHandler = () => {
  ...
  useEffect(() => {
    setMaxPageNumber(Math.ceil(result.length/pageSize));
  },[result]);
  ...
  return (
    <div className="chars">
      ...
      <div className="pagination">
        ...
        <p> {pageNumber} </p>
        <button onClick={() => setPageNumber(pageNumber === maxPageNumber ? maxPageNumber : pageNumber+1)} className="right-button" ><img className="arrow" src={Arrow} alt=""/></button>
      </div>
      <div className="cards">
